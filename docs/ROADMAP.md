...
3. Indexer pattern engine + MCP `structural_search` (codepulse-owned metavariable patterns; no ast-grep)
4. Daemon HTTP ingest (`POST /v1/batches`)
5. Python agent: `sys.monitoring` baseline aggregation + flush
6. MCP read tools: `get_function_runtime_summary`, `get_hot_paths`, `get_static_summary`, callers/callees
7. Demo app (FastAPI or Flask) + scripted scenario

**Exit criteria:** An AI agent can answer “what was hottest?”, “who calls X?”, and run a structural pattern search over Python sources.
...
3. Additional tree-sitter grammars for static index **and** structural search (search langs track grammars, not agent processes)
